...
WebSocket auth: token passed as ?token= query param (cookies can't be set
on ws:// easily from the browser, so we fall back to the query param which
is still validated by the same decode_token function).
"""
...
async def websocket_endpoint(
    websocket: WebSocket,
    user_id: int,
    token: str = Query(...),   # JWT passed as ?token=<jwt>
):
    """
