...
These are what FastAPI uses — separate from the DB models.
"""
...
class ScanRequest(BaseModel):
    ...
    scan_type: str = "full"   # "quick" | "full" | "ports_only"
...
class DeviceInfo(BaseModel):
    ...
    vendor: str = ""
    os_guess: str = ""
    status: str = "unknown"    # trusted | unknown | threat
    ports: List[PortInfo] = []
...
class AgentScanPayload(BaseModel):
    """Payload the local agent sends after running a scan."""
    agent_secret: str
    user_id: int
    network_range: str
    devices: List[DeviceInfo]
...
class PasswordCheckRequest(BaseModel):
    hash_prefix: str   # first 5 chars of SHA-1 hash — never the real password
...
class ChatRequest(BaseModel):
    ...
    scan_context: Optional[dict] = None   # current scan summary injected into system prompt
