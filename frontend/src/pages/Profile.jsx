...
// ── API calls (add these to api.js too) ───────────────────────────────────────
const profileApi = {
  get:            () => fetch('/api/auth/profile',          { credentials: 'include' }).then(r => r.json()),
  update:         (body) => fetch('/api/auth/profile',      { method: 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then(r => r.json()),
  changePassword: (body) => fetch('/api/auth/change-password', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then(r => r.json()),
  sendVerification: () => fetch('/api/auth/send-verification', { method: 'POST', credentials: 'include' }).then(r => r.json()),
  deleteAccount:  () => fetch('/api/auth/account',          { method: 'DELETE', credentials: 'include' }).then(r => r.json()),
}
...
function StatChip({ label, value, color }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 28, color, fontWeight: 700, lineHeight: 1 }}>{value}</div>
...
export default function Profile() {
  ...
  async function saveInfo(e) {
    ...
    try {
      ...
      if (res.detail?.includes('updated')) {
        setInfoMsg({ type: 'success', text: 'Profile updated successfully.' })
        if (res.username) auth_state.setUsername(res.username)
        await loadProfile()
      } else {
        setInfoMsg({ type: 'error', text: res.detail ?? 'Update failed.' })
      }
    } catch {
      setInfoMsg({ type: 'error', text: 'Network error. Please try again.' })
    } finally {
      setInfoSaving(false)
    }
  }
  ...
  async function savePassword(e) {
    ...
    try {
      ...
      if (res.detail?.includes('success')) {
        setPwMsg({ type: 'success', text: 'Password changed successfully.' })
        setPwForm({ current_password: '', new_password: '', confirm: '' })
      } else {
        setPwMsg({ type: 'error', text: res.detail ?? 'Failed to change password.' })
      }
    } catch {
  ...
  async function sendVerification() {
    ...
    try {
      ...
      setVerifyMsg({ type: 'error', text: 'Failed to send verification email.' })
    } finally {
  ...
  async function deleteAccount() {
    ...
    try {
      ...
      setDeleteMsg({ type: 'error', text: 'Failed to delete account. Try again.' })
    ...
    <div className="animate-in" style={{ maxWidth: 700 }}>
      ...
      <Section title="IDENTITY">
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <StatChip label="TOTAL SCANS"   value={profile?.total_scans   ?? 0} color="var(--blue)" />
          <StatChip label="DEVICES FOUND" value={profile?.total_devices ?? 0} color="var(--amber)" />
          <StatChip label="THREATS FOUND" value={profile?.total_threats ?? 0} color="var(--red-bright)" />
        </div>
      </Section>
            ...
            <div key={key}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  placeholder={key === 'current_password' ? 'Current password' : key === 'new_password' ? 'Min. 8 characters' : 'Repeat new password'}
                />
                <button
                  type="button"
